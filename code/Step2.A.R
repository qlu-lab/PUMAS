...
main <- function(){
    ...
    load("/z/Comp/lu_group/Members/zzhao/PUMAS_ext/GWAS_Atlas/LD/ld_1kg.RData")
    load("/z/Comp/lu_group/Members/zzhao/PUMAS_ext/GWAS_Atlas/LD/rs_1kg.RData")
    ...
    # PUMAS R2, k times
    pumas_tmp <- PUMAS.II.FUN.I(FunII.GWAS=matched_data$gwas_matched, FunII.Nt=partitions[2]*floor(min(matched_data$gwas_matched$N)),FunII.LD=matched_data$new_ld_blocks, FunII.LD.block=matched_data$ld_block_size, trait_name=trait_name, k=k, chr=chr)

    fwrite(data.frame(var.Y=pumas_tmp,N.t=partitions[2]*floor(min(matched_data$gwas_matched$N))),paste0(output_path,trait_name,".forEVAL",chr,".txt"),col.names=T,row.names=F,sep="\t",quote=F)
}
