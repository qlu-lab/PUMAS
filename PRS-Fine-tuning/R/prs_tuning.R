#source("/Users/yupeilin/PRS-Fine-tuning/R/CVFunII_v4.R")
set.seed(0)
...
FunI.PEhat<-function(FunI.beta_hat,FunI.test.Y,FunI.test.X){
  ...
  #if (! is.vector(FunI.XtY)){stop(print("X_t_Y should be a vector!"))}
  #if (! (is.matrix(FunI.XtX) | is.data.frame(FunI.XtX))){stop(print("X_t_X should be a Matrix!"))}
  #if (length(FunI.beta_hat)!=length(FunI.XtY) | length(FunI.beta_hat)!=nrow(FunI.XtX) | nrow(FunI.XtX)!=ncol(FunI.XtX)){stop(print("Please check dimensions of parameters!"))}
  #if (! isSymmetric(FunI.XtX)){stop(print("X_t_X is not symmetric!"))}
  #FunI.PE.term2 <- -2*FunI.beta_hat*FunI.XtY
  #FunI.PE.term3 <- apply(FunI.XtX*FunI.beta_hat,1,function(z){z*FunI.beta_hat})
  # if (! isSymmetric(FunI.PE.term3)){stop(print("Term.3 is not symmetric!"))}
  #FunI.PE <- (cumsum(FunI.PE.term2) + diag(apply(apply(FunI.PE.term3,2,cumsum),1,cumsum)))/FunI.nv
...
FunI.PEtilde<-function(FunI.XtY.tr, FunI.XtY.v, FunI.ntr, FunI.nv, FunI.Sigma,FunI.Var.Y){
  ...
  # if (is.matrix(FunI.Sigma) & !isSymmetric(FunI.Sigma)){stop(print("Sigma is not symmetric!"))}
  ...
  if(is.vector(FunI.Sigma)){
    ...
    #FunI.PE <- cumsum(FunI.PE.term2+FunI.PE.term3)
    #FunI.R2 <- -FunI.PE/FunI.Var.Y
...
#source("/Users/yupeilin/PRS-Fine-tuning/R/CVFunI_v4.R")
...
FunII.TildeRL<-function(FunII.beta, FunII.SE, FunII.sigma, FunII.N.samplesize, FunII.Nv, FunII.rep, FunII.Corr=F){
  ...
  if(FunII.Nv>=max(FunII.N.samplesize)){stop("Error: Nv is larger or equal than the maximum of N.samplesize!")}
  ...
  if (FunII.Corr==F){
    ...
    for (FunII.r in 1:FunII.rep){
      ...
      #FunII.XtY.train.temp <- t(x.train)%*%y.train
      FunII.XtY.test.temp <- FunII.XtY - FunII.XtY.train.temp
      #FunII.XtY.test.temp <- rnorm(n=length(FunII.beta), mean = (FunII.Nv/FunII.N.samplesize)*FunII.XtY, sd = sqrt((FunII.Ntr*FunII.Nv/FunII.N.samplesize)*FunII.VarXY))
      ...
      if(is.vector(FunII.sigma)){
        ...
        #FunII.PErr.Avg <- FunI.PEtilde(FunI.XtY.tr=FunII.XtY.train.temp[FunII.order.temp],
        #FunI.XtY.v=FunII.XtY.test.temp[FunII.order.temp],
        #FunI.ntr=FunII.Ntr, FunI.nv=FunII.Nv[FunII.order.temp],
        #FunI.Sigma=FunII.sigma.diag[FunII.order.temp],FunI.Var.Y=FunII.Var.Y)
      }else{
        FunII.PErr.Avg <- FunII.PErr.Avg + FunI.PEtilde(FunI.XtY.tr=FunII.XtY.train.temp[FunII.order.temp],
                                                        FunI.XtY.v=FunII.XtY.test.temp[FunII.order.temp],
                                                        FunI.ntr=FunII.Ntr, FunI.nv=FunII.Nv[FunII.order.temp],
    ...
    if(is.vector(FunII.sigma)){
      ...
      for (FunII.r in 1){
        ...
        #FunI.Sigma=FunII.sigma.diag[FunII.order.temp[,FunII.r]])/FunII.rep
