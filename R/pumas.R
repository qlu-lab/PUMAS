pumas.main<-function(input_path,output_path, beta_header,af_header,se_header,pvalue_header,sample_size,flag_plot){
  ...
  if (is.null(af_header)){print("Allele frequency is missing. Please provide the correct header for allele frequency.")}
  ...
  AF=data.real[,which(names(data.real)==af_header)]
  se=data.real[,which(names(data.real)==se_header)]
  pvalue=data.real[,which(names(data.real)==pvalue_header)]
  X_VAR=2*AF*(1-AF)
